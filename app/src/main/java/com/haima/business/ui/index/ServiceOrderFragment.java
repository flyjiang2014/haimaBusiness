 ...
 * 文件说明：
 */
public class ServiceOrderFragment extends BaseFragmentV4 {
    ...
    protected void initViewData() {
        for (int i = 0; i <5 ; i++) {
            ServiceOrderItemFragment serviceOrderItemFragment = ServiceOrderItemFragment.getInstance(i-1);
            mFragments.add(serviceOrderItemFragment);
        }
    ...
    public static ServiceOrderFragment getInstance() {
    ...
    public class MyFragmentAdapter extends FragmentStatePagerAdapter {
        ...
        private String[] types = {"全部", "待确认", "待支付", "待服务", "已完成"};
        FragmentManager fm;
    ...
    public void onEvent(EventBean data) {
        switch (data.getEvent()) {
            case EventBean.SERVICE_FINISH_SUCCESS:
                if(viewPager.getCurrentItem()== 0||viewPager.getCurrentItem()== 3||viewPager.getCurrentItem()== 4){//全部，待服务,已完成
